...
export default function MensagensAdmin() {
  ...
  const handleToggleReadStatus = async (id, currentStatus) => {
    try {
      console.log(`Tentando ${currentStatus ? 'desmarcar' : 'marcar'} mensagem como lida:`, id)
      ...
      if (currentStatus) {
        alert('Funcionalidade de desmarcar como lida ainda não está implementada no backend. Entre em contato com o desenvolvedor.')
      } else {
        alert('Erro ao marcar mensagem como lida: ' + error.message)
      }
    }
    ...
    <section className="min-h-screen w-full max-w-7xl mx-auto px-6 py-16">
      ...
      <div className="space-y-4">
        {filteredMensagens.map((mensagem) => (
          <Card key={mensagem.id} className={`shadow-sm ${!mensagem.read ? 'border-l-4 border-l-yellow-500 bg-yellow-50/30' : 'border-l-4 border-l-green-500 bg-green-50/30'}`}>
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-4">
                  <div className={`w-3 h-3 rounded-full ${mensagem.read ? 'bg-green-500' : 'bg-yellow-500'}`} />
                  <div>
                ...
                <div className="flex space-x-2">
                  <Button
                    ...
                    className={mensagem.read ? "text-green-600 hover:bg-green-50 border-green-200" : "text-yellow-600 hover:bg-yellow-50 border-yellow-200"}
              ...
              {mensagem.assunto && (
                <div className="mb-3">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">{mensagem.assunto}</p>
                </div>
              )}
      ...
      {filteredMensagens.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium mb-2">
            {filter === 'all' ? 'Nenhuma mensagem encontrada' :
